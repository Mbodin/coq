...
type kind = Term | Type | Cons | Proj | Mod
...
let add_duplicate, get_duplicate =
  ...
  let add mp l =
     ...
     Printf.printf "DEBUG: %s, %s -> %s \n%!" (ModPath.debug_to_string mp) (Label.to_string l) ren ;
...
let pp_ocaml_local k prefix mp rls olab =
  ...
  if not (visible_clash prefix k's) then dottify rls'
    ...
    (* Projections are linked to their associated type. *)
    let t = ?? (* TODO: Find the associated type *) in
    pp_duplicate Type prefix mp rls' olab (* FIXME: Is that right? *)
